...
;;; technically we should store this in the environment but it doesn't
;;; actually matter, as we only have the one set of expanders.
(eval-when (:compile-toplevel :load-toplevel :execute)
  (defparameter *atomic-expanders* (make-hash-table))

  (defun atomic-expander (symbol)
    (values (gethash symbol *atomic-expanders*)))
  (defun (setf atomic-expander) (expander symbol)
    (setf (gethash symbol *atomic-expanders*) expander)))
...
defaulting of ORDER is applied."
  ...
  (etypecase place
    (symbol
     (let ((info (cross-clasp:describe-variable place environment)))
       (etypecase info
         ...
         (trucler:symbol-macro-description
          (apply #'get-atomic-expansion
                 (cross-clasp:build-macroexpand-1 place environment)
                 keys))
         (trucler:lexical-variable-description
          (error "Lexical variable is not an atomic place: ~s" place))
         (trucler:constant-variable-description
          (error "Constant is not an atomic place: ~s" place))
         (null
          (error "Can't macroexpand atomic place for unknown variable: ~s"
                 place)))))
    (cons
     (let* ((name (car place))
       ...
       (if expander
           ...
           (multiple-value-bind (expansion expanded)
             ...
             (if expanded
                 ...
                 (error "Not an atomic place: ~s" place))))))))
...
;;; Installed as a setf expander in base.lisp.
(defun expand-atomic (whole env)
  (destructuring-bind (place &rest keys &key &allow-other-keys) (rest whole)
  ...
  (multiple-value-bind (vars vals old new read write cas)
    ...
    (let ((gitem (gensym "ITEM")) (bname (gensym "ATOMIC-PUSHNEW"))
      ...
      (loop for thing in keys
            if (cross-clasp:constantp thing env)
              do (push (cross-clasp:constant-form-value thing env) gkeys)
            else
