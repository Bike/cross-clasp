  ...
  (etypecase method
    (effective-reader
     (let* ((args (emf-params env))
            ...
            (slot (effective-slot method))
            (valuef
              (ecase (allocation slot)
                ((:instance) `(core:instance-ref arg ',(location slot))))))
       `(let* ((arg ,arg)
               (value ,valuef))
          (if (eq value (core:unbound))
              (slot-unbound (class-of arg) arg ',(name slot))
              value))))
    (effective-writer
     (let* ((args (emf-params env))
            (slot (effective-slot method))
       ...
       (ecase (allocation slot)
         ((:instance)
          `(setf (core:instance-ref ,obj ',(location slot)) ,value)))))
